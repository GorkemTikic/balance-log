...
function groupByTypeAndAsset(rows: Row[]): TotalsByType {
  ...
  for(const r of rows){ const key = r.type || "(unknown)"; (map.get(key) || map.set(key, []).get(key)!)!.push(r); }
  const out: TotalsByType = {};
  for(const [t, list] of map.entries()) out[t] = sumByAsset(list);
  return out;
}

function humanize(t: string) { return t.replace(/_/g, " ").replace(/\b([a-z])/g, s => s.toUpperCase()); }
  ...
  return (
    <div className="container">
      ...
      <TypeFilter
        ...
        counts={rowsByDateSymbol.reduce((acc:Record<string,number>, r)=>{ const k=r.type||"(unknown)"; acc[k]=(acc[k]||0)+1; return acc; }, {})}
      />
      ...
      {tab === "symbol" && (
        <section style={{ marginTop: 12 }}>
          <SymbolTable rows={rows.map(r => ({ symbol: r.symbol, asset: r.asset, type: r.type, amount: r.amount }))} />
        </section>
      )}
