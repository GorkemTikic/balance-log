...
export default function StoryDrawer({
  ...
  function parseBaseline(s: string): {
  ...
  // Summary (table under narrative)
  const summaryRows: SummaryRow[] = useMemo(
    () => buildSummaryRows(filterRowsForSummary(rows, anchorISO)),
    [rows, anchorISO],
  );

  // Audit (final balances, consistent with narrative rules)
  ...
  return (
    ...
    >
      ...
      >
        {/* Sticky header (wrapped buttons; fixed right padding to prevent cut-off) */}
        <div
        ...
        >
          ...
          <div className="btn-row" style={{ gap: 8, flexWrap: "wrap" }}>
            ...
            {tab === "audit" && (
              <button className="btn" onClick={() => copy(auditText)}>
                Copy Audit
              </button>
            )}
            {tab === "raw" && (
              <button className="btn" onClick={() => copy(rawPreview)}>
                Copy Raw
              </button>
            )}
            <select
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns:
                      "minmax(170px,1fr) minmax(130px,1fr)", // widened to avoid cut-off
                    gap: 8,
                    marginTop: 6,
                  }}
                >
                  <input
                    className="btn"
                    style={inputStyle}
                    placeholder="Amount (e.g. 2000 or -0.015)"
                    value={trAmount}
                      ...
                      <tr key={i} style={{ background: i % 2 ? "#fff" : "#fbfbfd" }}>
                        ...
                        <td
                          style={{
                            ...tdStyleMonoBold,
                            color:
                              r.net === 0 ? "#6b7280" : r.net > 0 ? "#047857" : "#b91c1c",
                          }}
                        >
                          {r.net}
                        </td>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div
                  style={{
                    display: "grid",
                    gridTemplateColumns:
                      "minmax(170px,1fr) minmax(130px,1fr)", // widened here too
                    gap: 8,
                    marginTop: 6,
                  }}
                >
...
const tdStyleMonoBold: React.CSSProperties = {
  ...tdStyleMono,
  fontWeight: 700,
};
...
function ChartLine({ data, height = 240 }: { data: LinePoint[]; height?: number }) {
  ...
  const yScale = (v: number) =>
    pad.t +
    (maxY === minY ? innerH / 2 : innerH - ((v - minY) / (maxY - minY)) * innerH);
...
// Restrict summary to same time window as narrative (when anchor set)
function filterRowsForSummary(rows: Row[], anchorISO?: string) {
