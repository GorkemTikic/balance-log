...
const setShow = (k: keyof Filters["show"], v: boolean) => onChange({ ...value, show: { ...value.show, [k]: v } });
...
<input className="btn" style={{ width: "100%", textAlign: "left", marginTop: 6 }} value={value.t0} onChange={(e) => set({ t0: e.target.value })} placeholder="YYYY-MM-DD HH:MM:SS" />
</label>
<label className="muted">End (UTC+0)
<input className="btn" style={{ width: "100%", textAlign: "left", marginTop: 6 }} value={value.t1} onChange={(e) => set({ t1: e.target.value })} placeholder="YYYY-MM-DD HH:MM:SS" />
</label>
<label className="muted">Symbol contains
<input className="btn" style={{ width: "100%", textAlign: "left", marginTop: 6 }} value={value.symbol} onChange={(e) => set({ symbol: e.target.value })} placeholder="e.g. BTC, ETH, PEPEUSDT" />
</label>
</div>
